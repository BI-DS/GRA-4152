...
* `utils.py` provides the way you can define the Adam optimizer for a given learning rate (and default values in all other parameters), and the function that you need to use to update the trainable variables in the encoder and decoder of your VAE. Remember that your encoder/decoder classes must inherit from `layers.Layer` and your VAE from `tf.keras.Model`. You also find a nice function to plot a grid of 100 images that you generate with the VAE, and the links to download all datasets. The test data set is needed to reconstruct images (using the posterior distribution)  after you have trained the model. The labels are only needed if you want to assign colors to the scatter plot of the latent space. The safest way to download the data is with `wget` that should be called from your `DataLoader` class.
